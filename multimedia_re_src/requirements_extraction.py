...
try:
    ...
    logging.warning(
        "scikit-learn not available. Some analysis features may be limited."
    )
...
class RequirementsExtractor:
    ...
    def __init__(self):
        ...
        self.modality_weights = {"audio": 0.35, "video": 0.35, "image": 0.30}
...
def extract_requirements_from_multimedia(
    ...
    try:
        ...
        audio_requirements = extract_audio_requirements(
            multimedia_data.get("audio", {})
        )
        video_requirements = extract_video_requirements(
            multimedia_data.get("video", {})
        )
        image_requirements = extract_image_requirements(
            multimedia_data.get("image", {})
        )
        ...
        logger.info(
            f"Requirements extraction completed. {len(final_requirements)} requirements extracted."
        )
        return result
...
def validate_cross_modal_evidence(
    ...
    # Sort by confidence
    validated_requirements.sort(
        key=lambda x: x.get("confidence_score", 0), reverse=True
    )

    logger.info(
        f"Cross-modal validation completed. {len(validated_requirements)} validated requirements."
    )
...
def classify_requirements(requirements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for req in requirements:
        ...
        functional_score = calculate_pattern_score(
            description, extractor.functional_patterns
        )

        # Calculate non-functional score
        non_functional_score = calculate_pattern_score(
            description, extractor.non_functional_patterns
        )
...
def generate_requirements_report(
    requirements_data: Dict[str, Any], output_format: str = "json"
) -> str:
    """
...
def create_traceability_matrix(
    requirements: List[Dict[str, Any]], multimedia_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def extract_single_image_requirements(
...
def group_similar_requirements(
...
def calculate_single_requirement_confidence(req: Dict[str, Any]) -> float:
    ...
    if modality == "audio":
        ...
        if any(
            word in req.get("description", "").lower()
...
def calculate_cross_modal_confidence(
    group: List[Dict[str, Any]], modalities: List[str]
) -> float:
    ...
    individual_confidences = [
        calculate_single_requirement_confidence(req) for req in group
    ]
    ...
    cross_modal_confidence = (
        avg_individual_confidence * 0.6 + modality_bonus + consistency_bonus * 0.2
    )
...
def merge_requirement_group(
    group: List[Dict[str, Any]], group_id: int
) -> Dict[str, Any]:
    """Merge a group of similar requirements into single requirement."""
    # Use the most detailed description
    descriptions = [
        req.get("description", "") for req in group if req.get("description")
    ]
    merged_description = (
        max(descriptions, key=len) if descriptions else "No description available"
    )
    ...
    merged_req = {
        ...
        "original_requirements": [
            req.get("id", f"orig_{i}") for i, req in enumerate(group)
        ],
    }
...
def generate_classification_summary(
    ...
    summary = {
        ...
        "functional_percentage": (
            (classification_counts.get("functional", 0) / total_reqs * 100)
            if total_reqs > 0
            else 0
        ),
        "non_functional_percentage": (
            (classification_counts.get("non_functional", 0) / total_reqs * 100)
            if total_reqs > 0
            else 0
        ),
    }
    ...
    nf_reqs = [
        req for req in requirements if req.get("classification") == "non_functional"
    ]
    nf_sub_classifications = [
        req.get("sub_classification", "unknown") for req in nf_reqs
    ]
...
def generate_final_requirement_set(
...
def determine_moscow_priority(req: Dict[str, Any]) -> str:
    ...
    if any(
        word in description
        for word in ["security", "authentication", "critical", "essential"]
    ):
        return "Must"
...
def extract_sentiment_based_requirements(
...
def extract_keyword_based_requirements(
    keywords: List[str], entities: List[Any]
) -> List[Dict[str, Any]]:
...
def extract_hesitation_requirements(
...
def extract_confusion_requirements(
    confusion_indicators: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
...
def create_source_data_summary(multimedia_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {"modalities_processed": len(multimedia_data), "modality_details": {}}
...
def calculate_extraction_quality_metrics(
    ...
    cross_modal_reqs = [
        req for req in requirements if len(req.get("supporting_modalities", [])) > 1
    ]
