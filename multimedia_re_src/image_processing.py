...
def extract_text_from_image(
    image: np.ndarray, languages: str = "eng+ces"
) -> Dict[str, Any]:
    """
...
def map_spatial_annotations(
    image: np.ndarray, annotations: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """
...
def generate_requirements_from_image(
    text: Dict[str, Any],
...
def merge_overlapping_detections(
    detections: List[Dict[str, Any]], overlap_threshold: float = 0.5
) -> List[Dict[str, Any]]:
    ...
    for i, det1 in enumerate(detections):
        ...
        for j, det2 in enumerate(detections[i + 1 :], i + 1):
            ...
            if (
                calculate_overlap(det1.get("bbox"), det2.get("bbox"))
                > overlap_threshold
            ):
...
def get_grid_coordinates(
    x: int, y: int, w: int, h: int, width: int, height: int, grid_size: int
) -> Tuple[int, int]:
...
def are_annotations_close(
    ann1: Dict[str, Any], ann2: Dict[str, Any], distance_threshold: float = 100.0
) -> bool:
...
def extract_ui_based_requirements(
    ui_elements: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Extract requirements from UI element analysis."""
    requirements = []
...
def extract_annotation_based_requirements(
    annotations: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Extract requirements from user annotations."""
    requirements = []
...
def extract_visual_design_requirements(
    visual_analysis: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """Extract requirements from visual design analysis."""
    requirements = []
...
def determine_requirement_priority(
    requirement: Dict[str, Any], annotations: List[Dict[str, Any]]
) -> str:
