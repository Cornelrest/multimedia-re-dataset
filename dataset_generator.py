...
class MultimediaREDatasetGenerator:
            ...
            participants.append(
                {
                    ...
                    "technical_background": np.random.choice(
                        ["Low", "Medium", "High"], p=[0.2, 0.5, 0.3]
                    ),
                    "university": np.random.choice(
                        ["TBU", "CTU", "UEP"], p=[0.4, 0.3, 0.3]
                    ),
                    ...
                    "elearning_experience": np.random.choice(
            ...
            participants.append(
                {
                    ...
                    "technical_background": np.random.choice(
                        ["Low", "Medium", "High"], p=[0.15, 0.6, 0.25]
                    ),
                    "university": np.random.choice(
                        ["TBU", "CTU", "UEP"], p=[0.4, 0.3, 0.3]
                    ),
            ...
            sessions.append(
                {
                    ...
                    "completion_status": np.random.choice(
                        ["completed", "completed", "completed", "partial"],
                        p=[0.9, 0.05, 0.03, 0.02],
                    ),
                    "satisfaction_score": np.random.normal(
                        4.8 if group == "control" else 6.1,
    ...
    def generate_ground_truth_requirements(self):
                ...
                requirements.append(
                    {
                        ...
                        "identified_by_control": np.random.choice(
                            [0, 1], p=[0.29, 0.71]
                        ),  # 71% identification rate
                        "identified_by_treatment": np.random.choice(
                            [0, 1], p=[0.08, 0.92]
                        ),  # 92% identification rate
        ...
        for category in nf_categories:
            ...
            for i in range(num_reqs):
                ...
                req_id += 1
                if (
                    len([r for r in requirements if r["type"] == "non-functional"])
                    >= 54
                ):
                    break
            ...
            results.append(
                {
                    "participant_id": participant_id,
                    "group": group,
                    "requirements_identified": max(
                        20,
                        np.random.normal(
                            stats["requirements_identified"][0],
                            stats["requirements_identified"][1],
                        ),
                    ),
                    "precision": np.clip(
                        np.random.normal(stats["precision"][0], stats["precision"][1]),
                        0,
                        1,
                    ),
                    "recall": np.clip(
                        np.random.normal(stats["recall"][0], stats["recall"][1]), 0, 1
                    ),
                    "f1_score": np.clip(
                        np.random.normal(stats["f1_score"][0], stats["f1_score"][1]),
                        0,
                        1,
                    ),
                    ...
                    "session_duration": np.random.normal(
                        60 if group == "control" else 90, 10
                    ),
                    "analysis_time_minutes": np.random.normal(
                        180 if group == "control" else 45, 20
                    ),
                }
            )
    ...
    def generate_summary_statistics(
        self, demographics_df, evaluation_df, ground_truth_df, multimedia_df
    ):
        ...
        summary_stats = {
            "dataset_info": {
                ...
                "control_group_size": len(
                    demographics_df[demographics_df["group"] == "control"]
                ),
                "treatment_group_size": len(
                    demographics_df[demographics_df["group"] == "treatment"]
                ),
            },
            "requirements": {
                "total_requirements": len(ground_truth_df),
                "functional_requirements": len(
                    ground_truth_df[ground_truth_df["type"] == "functional"]
                ),
            ...
            "multimedia": {
                ...
                "audio_files": len(
                    multimedia_df[multimedia_df["file_type"] == "audio"]
                ),
                "video_files": len(
                    multimedia_df[multimedia_df["file_type"] == "video"]
                ),
                "image_files": len(
                    multimedia_df[multimedia_df["file_type"] == "image"]
                ),
            },
            "session_stats": {
                "average_duration_control": evaluation_df[
                    evaluation_df["group"] == "control"
                ]["session_duration"].mean(),
                "average_duration_treatment": evaluation_df[
                    evaluation_df["group"] == "treatment"
                ]["session_duration"].mean(),
            },
            "key_results": {
                "requirements_improvement": (
                    (
                        evaluation_df[evaluation_df["group"] == "treatment"][
                            "requirements_identified"
                        ].mean()
                        - evaluation_df[evaluation_df["group"] == "control"][
                            "requirements_identified"
                        ].mean()
                    )
                    / evaluation_df[evaluation_df["group"] == "control"][
    ...
    def print_generation_summary(self, datasets, summary_stats):
        ...
        print(
            f"Treatment group: {summary_stats['dataset_info']['treatment_group_size']}"
        )
        print(
            f"Total requirements: {summary_stats['requirements']['total_requirements']}"
        )
        print(
            f"Functional requirements: {summary_stats['requirements']['functional_requirements']}"
        )
        print(
