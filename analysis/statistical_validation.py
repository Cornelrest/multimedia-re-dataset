...
try:
    ...
    logging.warning(
        "statsmodels not available. Some advanced statistical tests may be limited."
    )
...
try:
    ...
    logging.warning(
        "matplotlib/seaborn not available. Plotting functions will be disabled."
    )
...
class StatisticalValidator:
    ...
    def validate_experiment(
        ...
        # Overall conclusion
        validation_results["overall_conclusion"] = self.generate_overall_conclusion(
            validation_results
        )
    ...
    def check_statistical_assumptions(self, data: ExperimentalData) -> Dict[str, Any]:
        ...
        assumptions["normality"] = {
            ...
            "both_normal": (
                control_shapiro.pvalue > self.alpha
                and treatment_shapiro.pvalue > self.alpha
            ),
        }
        ...
        if not assumptions["normality"]["both_normal"]:
            assumptions["recommendations"].append(
                "Consider non-parametric tests due to non-normality"
            )

        if not assumptions["homogeneity_of_variance"]["equal_variances"]:
            assumptions["recommendations"].append(
                "Use Welch's t-test due to unequal variances"
            )

        if not assumptions["sample_size"]["adequate_size"]:
            assumptions["recommendations"].append(
                "Sample size may be inadequate for reliable inference"
            )

        return assumptions

    def perform_statistical_tests(
        self, data: ExperimentalData, assumptions: Dict[str, Any]
    ...
    def independent_t_test(
        self, data: ExperimentalData, equal_var: bool = True
    ) -> StatisticalResult:
        """
        ...
        statistic, p_value = ttest_ind(
            data.treatment_group, data.control_group, equal_var=equal_var
        )
    ...
    def mann_whitney_u_test(self, data: ExperimentalData) -> StatisticalResult:
        ...
        statistic, p_value = mannwhitneyu(
            data.treatment_group, data.control_group, alternative="two-sided"
        )
    ...
    def bootstrap_mean_difference(
        self, data: ExperimentalData, n_bootstrap: int = 10000
    ) -> StatisticalResult:
        """
        ...
        for _ in range(n_bootstrap):
            ...
            boot_treatment = np.random.choice(
                data.treatment_group, n_treatment, replace=True
            )
    ...
    def calculate_effect_sizes(self, data: ExperimentalData) -> Dict[str, float]:
        ...
        # Cohen's d
        effect_sizes["cohens_d"] = self.cohens_d(
            data.treatment_group, data.control_group
        )

        # Hedges' g (bias-corrected)
        effect_sizes["hedges_g"] = self.hedges_g(
            data.treatment_group, data.control_group
        )

        # Glass's delta
        effect_sizes["glass_delta"] = self.glass_delta(
            data.treatment_group, data.control_group
        )

        # Common language effect size
        effect_sizes["cles"] = self.common_language_effect_size(
            data.treatment_group, data.control_group
        )

        # Probability of superiority
        effect_sizes["prob_superiority"] = self.probability_of_superiority(
            data.treatment_group, data.control_group
        )
    ...
    def correct_multiple_comparisons(
        ...
        if not STATSMODELS_AVAILABLE:
            logger.warning(
                "statsmodels not available. Multiple comparisons correction limited."
            )
            return {}
        ...
        corrections["bonferroni"] = {
            ...
            "significant_metrics": [
                name for name, rejected in zip(metric_names, bonf_rejected) if rejected
            ],
        }
        ...
        corrections["fdr_bh"] = {
            ...
            "significant_metrics": [
                name for name, rejected in zip(metric_names, fdr_rejected) if rejected
            ],
        }
        ...
        corrections["holm"] = {
            ...
            "significant_metrics": [
                name for name, rejected in zip(metric_names, holm_rejected) if rejected
            ],
        }
    ...
    def generate_validation_summary(
        ...
        for metric, assumptions in validation_results.get(
            "assumption_checks", {}
        ).items():
            violations = []
            if not assumptions["normality"]["both_normal"]:
            ...
            if violations:
                summary["assumption_violations"].append(
                    {"metric": metric, "violations": violations}
                )
    ...
    def generate_overall_conclusion(self, validation_results: Dict[str, Any]) -> str:
        ...
        conclusion += (
            f"{significant_count} metrics with statistically significant differences "
        )
        ...
        adequate_power_count = sum(
            1
            for power_data in summary.get("power_summary", {}).values()
        ...
        if mc_results:
            bonf_significant = len(
                mc_results.get("bonferroni", {}).get("significant_metrics", [])
            )
            conclusion += f"After Bonferroni correction for multiple comparisons, "
            conclusion += (
                f"{bonf_significant} metrics remained statistically significant.\n\n"
            )

        # Final recommendation
        if significant_count > 0 and large_effects > 0:
            conclusion += (
                "CONCLUSION: Strong evidence supports the effectiveness of the "
            )
            conclusion += (
                "multimedia-enhanced approach with both statistical significance "
            )
            conclusion += "and practical importance."
        elif significant_count > 0:
            conclusion += (
                "CONCLUSION: Moderate evidence supports the multimedia-enhanced "
            )
            conclusion += (
                "approach with statistical significance but limited effect sizes."
            )
        else:
            conclusion += (
                "CONCLUSION: Insufficient evidence to support the multimedia-enhanced "
            )
            conclusion += (
                "approach. Consider methodological improvements or larger sample sizes."
            )
    ...
    def cohens_d(self, treatment: np.ndarray, control: np.ndarray) -> float:
        ...
        pooled_std = np.sqrt(
            ((n1 - 1) * np.var(treatment, ddof=1) + (n2 - 1) * np.var(control, ddof=1))
            / (n1 + n2 - 2)
        )
    ...
    def common_language_effect_size(
        self, treatment: np.ndarray, control: np.ndarray
    ) -> float:
    ...
    def mean_difference_ci(
        self, treatment: np.ndarray, control: np.ndarray, confidence: float = 0.95
    ) -> Tuple[float, float]:
        """Calculate confidence interval for mean difference."""
    ...
    def interpret_t_test(
        self, statistic: float, p_value: float, effect_size: float
    ) -> str:
    ...
    def interpret_mann_whitney(
        ...
        return (
            f"Non-parametric test is {significance} (p = {p_value:.4f}). "
    ...
    def interpret_power_analysis(self, power_results: Dict[str, Any]) -> str:
        ...
        if observed_power >= 0.8:
            ...
            interpretation += (
                "Moderate power - results should be interpreted with caution."
            )
        else:
        ...
        if "required_n_80_power" in power_results:
            ...
            interpretation += (
                f" Required sample size for 80% power: {required_n:.0f} per group."
            )
...
def calculate_cohens_kappa(
    expert1_reqs: List[str], expert2_reqs: List[str], all_possible_reqs: List[str]
) -> Dict[str, float]:
    """
    ...
    expert1_binary = np.array(
        [1 if req in expert1_reqs else 0 for req in all_possible_reqs]
    )
    expert2_binary = np.array(
        [1 if req in expert2_reqs else 0 for req in all_possible_reqs]
    )
    ...
    expected_agreement = p1_positive * p2_positive + (1 - p1_positive) * (
        1 - p2_positive
    )
...
def validate_multimedia_re_results(
