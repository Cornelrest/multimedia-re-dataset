...
class RequirementsAnalyzer:
    ...
    def load_data(self):
        ...
        try:
            self.participants = pd.read_csv(os.path.join(self.data_dir, 'participants.csv'))
            self.results = pd.read_csv(os.path.join(self.data_dir, 'participant_results.csv'))
            self.ground_truth = pd.read_csv(os.path.join(self.data_dir, 'ground_truth_requirements.csv'))
            
            # Multimedia data (treatment group only)
            self.audio_data = pd.read_csv(os.path.join(self.data_dir, 'audio_analysis.csv'))
            self.video_data = pd.read_csv(os.path.join(self.data_dir, 'video_analysis.csv'))
            self.image_data = pd.read_csv(os.path.join(self.data_dir, 'image_analysis.csv'))
            
            # Cost and metadata
            self.cost_data = pd.read_csv(os.path.join(self.data_dir, 'cost_analysis.csv'))
    ...
    def analyze_core_findings(self) -> Dict:
        ...
        completeness_improvement = ((treatment_mean_reqs - control_mean_reqs) / control_mean_reqs) * 100
        ...
        for metric in accuracy_metrics:
            ...
            print(f"   {metric.capitalize()}: {control_mean:.3f} → {treatment_mean:.3f} ({improvement:+.1f}%)")
        ...
        print(f"   Functional: {func_control} → {func_treatment} ({func_improvement:+.1f}%)")
        print(f"   Non-functional: {nf_control} → {nf_treatment} ({nf_improvement:+.1f}%)")
    ...
    def perform_statistical_tests(self) -> Dict:
        ...
        for metric, description in test_metrics.items():
            ...
            print(f"   t({len(control_group) + len(treatment_group) - 2}) = {t_stat:.3f}, {p_display}")
    ...
    def analyze_multimedia_effectiveness(self) -> Dict:
        ...
        audio_stats = {
            ...
            'avg_requirements_per_segment': self.audio_data['requirements_extracted'].mean(),
            'total_requirements_extracted': self.audio_data['requirements_extracted'].sum()
        }
        ...
        print(f"   Requirements per segment: {audio_stats['avg_requirements_per_segment']:.1f}")
        ...
        video_stats = {
            ...
            'avg_requirements_per_segment': self.video_data['requirements_extracted'].mean(),
            'avg_interactions_detected': self.video_data['interactions_detected'].mean(),
        ...
        print(f"   Requirements per segment: {video_stats['avg_requirements_per_segment']:.1f}")
        print(f"   Interactions detected: {video_stats['avg_interactions_detected']:.1f}")
        ...
        image_stats = {
            ...
            'avg_requirements_per_image': self.image_data['requirements_extracted'].mean(),
        ...
        print(f"   Requirements per image: {image_stats['avg_requirements_per_image']:.1f}")
    ...
    def _create_core_comparison_plot(self, output_dir: str):
        ...
        fig.suptitle('Core Research Findings: Control vs Treatment Groups', fontsize=16, fontweight='bold')
        
        control_group = self.results[self.results['group_assignment'] == 'Control']
        treatment_group = self.results[self.results['group_assignment'] == 'Treatment']
        
        # Requirements Identified
        sns.boxplot(data=self.results, x='group_assignment', y='requirements_identified', ax=ax1)
        ...
        precision_data = pd.concat([
            pd.DataFrame({'Group': 'Control', 'Precision': control_group['precision']}),
            pd.DataFrame({'Group': 'Treatment', 'Precision': treatment_group['precision']})
        ])
        sns.boxplot(data=precision_data, x='Group', y='Precision', ax=ax2)
        ax2.set_title('Precision Scores (RQ2)', fontweight='bold')
        
        # Satisfaction Scores
        sns.boxplot(data=self.results, x='group_assignment', y='satisfaction_score', ax=ax3)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'core_findings_comparison.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_distribution_plots(self, output_dir: str):
        ...
        fig.suptitle('Statistical Distributions by Group', fontsize=16, fontweight='bold')
        ...
        for idx, (metric, title) in enumerate(metrics):
            ...
            control_data = self.results[self.results['group_assignment'] == 'Control'][metric]
            treatment_data = self.results[self.results['group_assignment'] == 'Treatment'][metric]
            
            ax.hist(control_data, alpha=0.7, label='Control', bins=10, color='red', density=True)
            ax.hist(treatment_data, alpha=0.7, label='Treatment', bins=10, color='blue', density=True)
    ...
    def _create_requirement_type_plots(self, output_dir: str):
        ...
        ax1.bar(x, non_functional_counts, width, bottom=functional_counts, label='Non-Functional', color='lightcoral')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'requirement_types_analysis.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_multimedia_plots(self, output_dir: str):
        ...
        fig.suptitle('Multimedia Modality Effectiveness', fontsize=16, fontweight='bold')
        ...
        axes[0, 0].bar(modalities, confidence_scores, color=['lightblue', 'lightgreen', 'lightcoral'])
        ...
        axes[0, 1].bar(modalities, requirements_extracted, color=['lightblue', 'lightgreen', 'lightcoral'])
        ...
        processing_times = pd.concat([
            pd.DataFrame({'Modality': 'Audio', 'Time': self.audio_data['processing_time_seconds']}),
            pd.DataFrame({'Modality': 'Video', 'Time': self.video_data['processing_time_seconds']}),
            pd.DataFrame({'Modality': 'Image', 'Time': self.image_data['processing_time_seconds']})
        ])
        ...
        axes[1, 1].bar(modalities, req_per_point, color=['lightblue', 'lightgreen', 'lightcoral'])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'multimedia_effectiveness.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_stakeholder_analysis(self, output_dir: str):
        ...
        stakeholder_data = self.results.groupby(['stakeholder_type', 'group_assignment'])['requirements_identified'].mean().unstack()
        stakeholder_data.plot(kind='bar', ax=axes[0, 0], color=['red', 'blue'], alpha=0.7)
        axes[0, 0].set_title('Average Requirements by Stakeholder Type', fontweight='bold')
        axes[0, 0].set_ylabel('Requirements Identified')
        axes[0, 0].legend(title='Group')
        axes[0, 0].tick_params(axis='x', rotation=45)
        
        # Satisfaction by stakeholder type
        stakeholder_satisfaction = self.results.groupby(['stakeholder_type', 'group_assignment'])['satisfaction_score'].mean().unstack()
        stakeholder_satisfaction.plot(kind='bar', ax=axes[0, 1], color=['red', 'blue'], alpha=0.7)
        axes[0, 1].set_title('Average Satisfaction by Stakeholder Type', fontweight='bold')
        axes[0, 1].set_ylabel('Satisfaction Score')
        axes[0, 1].legend(title='Group')
        axes[0, 1].tick_params(axis='x', rotation=45)
        
        # F1-Score by stakeholder type
        stakeholder_f1 = self.results.groupby(['stakeholder_type', 'group_assignment'])['f1_score'].mean().unstack()
        ...
        axes[1, 1].pie(stakeholder_counts.values, labels=stakeholder_counts.index, autopct='%1.1f%%',
                       colors=['lightblue', 'lightgreen', 'lightcoral'])
        axes[1, 1].set_title('Stakeholder Type Distribution', fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'stakeholder_analysis.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_time_analysis(self, output_dir: str):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle('Processing Time and Efficiency Analysis', fontsize=16, fontweight='bold')
        ...
        time_data = pd.concat([
            pd.DataFrame({'Group': 'Control', 'Total Time': control_group['total_time_min']}),
            pd.DataFrame({'Group': 'Treatment', 'Total Time': treatment_group['total_time_min']})
        ])
        ...
        time_components = ['data_collection_time_min', 'analysis_time_min', 'validation_time_min']
        ...
        axes[0, 1].bar(x - width/2, control_times, width, label='Control', color='red', alpha=0.7)
        axes[0, 1].bar(x + width/2, treatment_times, width, label='Treatment', color='blue', alpha=0.7)
        ...
        control_efficiency = control_group['requirements_identified'] / control_group['total_time_min']
        treatment_efficiency = treatment_group['requirements_identified'] / treatment_group['total_time_min']
        
        efficiency_data = pd.concat([
            pd.DataFrame({'Group': 'Control', 'Efficiency': control_efficiency}),
            pd.DataFrame({'Group': 'Treatment', 'Efficiency': treatment_efficiency})
        ])
        ...
        setup_costs = self.cost_data[self.cost_data['category'] == 'Setup Cost']['amount_usd'].sum()
        project_savings = self.cost_data[self.cost_data['category'] == 'Per-Project Savings']['amount_usd'].sum()
        
        projects = np.arange(1, 11)
        cumulative_savings = projects * project_savings - setup_costs
        
        axes[1, 1].plot(projects, cumulative_savings, marker='o', linewidth=2, markersize=6, color='green')
        axes[1, 1].axhline(y=0, color='red', linestyle='--', alpha=0.7, label='Break-even')
        axes[1, 1].fill_between(projects, cumulative_savings, 0, where=(cumulative_savings >= 0), 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'time_efficiency_analysis.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def generate_comprehensive_report(self, output_dir: str = "analysis_output") -> str:
        ...
        with open(report_file, 'w') as f:
            f.write(self._generate_report_content(core_findings, statistical_results, multimedia_analysis))
        
        print(f"✅ Comprehensive report saved to: {report_file}")
        return report_file

    def _generate_report_content(self, core_findings: Dict, statistical_results: Dict, multimedia_analysis: Dict) -> str:
