...
class TestRequirementsDatasetGenerator:
    ...
    def test_generate_ground_truth_requirements(self, generator):
        ...
        # Check required columns
        required_columns = [
            'requirement_id', 'type', 'category', 'priority', 
            'complexity', 'description', 'expert_consensus'
        ]
        ...
        assert type_counts['Non-Functional'] == generator.non_functional_requirements_total
    ...
    def test_generate_participant_results(self, generator):
        ...
        assert treatment_group['requirements_identified'].mean() > control_group['requirements_identified'].mean()
        assert treatment_group['precision'].mean() > control_group['precision'].mean()
        assert treatment_group['satisfaction_score'].mean() > control_group['satisfaction_score'].mean()
    ...
    def test_custom_parameters(self):
        ...
        assert 90 <= control_mean <= 110, f"Control mean {control_mean} not near expected 100"
        assert 140 <= treatment_mean <= 160, f"Treatment mean {treatment_mean} not near expected 150"
...
class TestDatasetValidator:
    ...
    def test_validate_ground_truth_requirements(self, validator):
        ...
        assert validator.validation_results['ground_truth']['non_functional_count'] == 54
...
class TestIntegration:
    ...
    def test_full_pipeline(self, temp_dir):
        ...
        improvement = ((treatment_group['requirements_identified'].mean() - 
                       control_group['requirements_identified'].mean()) / 
                      control_group['requirements_identified'].mean()) * 100
        
        assert 20 <= improvement <= 30, f"Improvement {improvement:.1f}% not in expected range"
    ...
    def test_data_consistency(self, temp_dir):
        ...
        treatment_participants = participants[participants['group_assignment'] == 'Treatment']['participant_id']
        audio_participants = set(audio_data['participant_id'].str[:4])  # First 4 chars of participant ID
